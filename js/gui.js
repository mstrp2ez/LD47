...
(function(){
	...
	const PLAYER_PARTY_KEY="player_party";
	class Widget{
		constructor(params){
		...
		setActive(active){
			this.active=active;
			if(this.onEnterCustom!==false&&active){
				eval(this.onEnterCustom);
			}
			if(this.onLeaveCustom!==false&&!active){
				eval(this.onLeaveCustom);
			}
	...
	Widget.prototype.calculateCoordinates=function(){
		...
		if(this.hasParent()){
			let pwc=this.parent.calculateCoordinates();
			wc.x+=pwc.x;
			wc.y+=pwc.y;
		}else{
			wc.x=this.x;
